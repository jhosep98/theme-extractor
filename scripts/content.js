...
function extractThemeData() {
  ...
  const themeData = {
    ...
    typography: {
      ...
      fontSize: Number.parseFloat(bodyStyles.fontSize), // Base font size
      htmlFontSize: Number.parseFloat(rootStyles.fontSize), // Usually 16px unless specified
      pxToRem: (px) => `${px / Number.parseFloat(rootStyles.fontSize)}rem`, // Helper
      h1: {},
      h2: {},
      h3: {},
      h4: {},
    ...
    for (const selector of colorRoles[role]) {
      ...
      if (element) {
        ...
        if (color && color !== 'rgba(0, 0, 0, 0)') {
          // Ignore transparent
          if (role === 'textSecondary') themeData.palette.text.secondary = color
          else if (role === 'backgroundPaper') themeData.palette.background.paper = color

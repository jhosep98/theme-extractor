document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!resultDiv || !copyButton || !notificationContainer || !notificationText) {
    console.error('ThemeExtractor: Required DOM elements not found in popup.html')
    return
  }
  ...
  try {
    ...
    if (response?.themeData) {
      ...
      showNotification('No theme data received or unexpected response from page.', false)
      resultDiv.textContent = 'Extraction failed.'
    }
  } catch (error) {
    console.error('ThemeExtractor: Error during extraction:', error)
    showNotification(`Error: ${error.message || 'Unknown error during extraction.'}`, false)
    resultDiv.textContent = 'Extraction failed.'
  }
})
